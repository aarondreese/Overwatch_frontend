...
async function handleGet(req, res) {
  ...
  if (id) {
    ...
    const dqChecksResult = await executeQuery(dqChecksQuery, {
      id: parseInt(id),
    });
    ...
    for (const check of dqChecksResult.recordset) {
      ...
      const schedulesResult = await executeQuery(schedulesQuery, {
        dqCheckId: check.id,
      });
...
async function handlePost(req, res) {
  ...
  if (!sourceSystemId || !domainName) {
    return apiResponse.badRequest(
      res,
      "Source System ID and Domain Name are required"
    );
  }
  ...
  if (duplicateCheck.recordset.length > 0) {
    return apiResponse.badRequest(
      res,
      "A domain with this name already exists for this source system"
    );
  }
