...
type ImportExportResponse = ConfigExportResponse | ConfigImportResponse;

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ImportExportResponse>
): Promise<void> {
  const { method } = req;
...
async function handleExport(
  req: NextApiRequest,
  res: NextApiResponse<ConfigExportResponse>
): Promise<void> {
  try {
    const query = `
      SELECT 
    ...
    const configs = result.recordset as Array<{
    ...
    const exportData = {
      ...
      configurations: regularConfigs.map(
        (c): ConfigExportItem => ({
...
async function handleImport(
  req: NextApiRequest,
  res: NextApiResponse<ConfigImportResponse>
): Promise<void> {
  try {
    const { importData, validateOnly = true } =
      req.body as ConfigImportRequest;
    ...
    const currentConfigs = currentResult.recordset as CurrentConfigRecord[];

    // Analyze differences
    const newRecords: NewRecord[] = [];
    const updatedRecords: UpdatedRecord[] = [];
    const unchangedRecords: UnchangedRecord[] = [];
