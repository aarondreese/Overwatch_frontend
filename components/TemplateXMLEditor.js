...
const HTMLTemplateEditor = memo(function HTMLTemplateEditor({
  value,
  onChange,
  placeholder,
}) {
...
export default function TemplateXMLEditor({
    ...
    try {
      ...
      collections.forEach((collection, index) => {
        const collectionNameElement =
          collection.querySelector("collectionname");
        ...
        if (collectionNameElement) {
          ...
          const groupByValue = groupByElement
            ? groupByElement.textContent
            : null;
          const instanceName = instanceNameElement
            ? instanceNameElement.textContent
            : null;
          ...
          if (hierarchyLevel) {
            ...
            if (groupByValue) {
              ...
              const actualColumn = viewColumns?.find(
                (col) =>
                  col.columnName.toLowerCase() === groupByValue.toLowerCase()
              );

              const actualGroupByValue = actualColumn
              ...
              if (actualColumn) {
                console.log(
                  `✅ Successfully set groupBy for ${collectionName} (${hierarchyLevel.variable}): ${groupByValue} -> ${actualGroupByValue}`
                );
              } else {
                console.log(
                  `⚠️ GroupBy column "${groupByValue}" not found in view columns, using as-is`
                );
              }
            } else {
            ...
            fields.forEach((field) => {
              ...
              if (nameElement && columnElement) {
                ...
                const actualColumnName = actualColumn
                  ? actualColumn.columnName
                  : xmlColumnName;
                ...
                if (actualColumn && actualColumn.columnName !== xmlColumnName) {
                  console.log(
                    `📝 Column mapping case corrected: ${xmlColumnName} -> ${actualColumnName}`
                  );
                }
              }
            });
            ...
            console.error(
              `❌ Could not find hierarchy level for collection: ${collectionName}, instanceName: ${instanceName}`
            );
            console.log(
              "Available hierarchy levels:",
              updatedHierarchy.map((h) => ({
      ...
      updatedHierarchy.forEach((level) => {
        if (level.xmlFields) {
          level.xmlFields.forEach((xmlField) => {
            ...
            const existsInVariables = level.variables.some(
              (v) => v.templateVar === xmlField.templateVar
            );
            if (!existsInVariables) {
              ...
              const existingVar = level.variables.find(
                (v) => v.templateVar === xmlField.templateVar
              );
              if (existingVar) {
      ...
      console.log(
        "Final updated hierarchy with groupBy values and hidden fields:",
        updatedHierarchy.map((h) => ({
          collection: h.collection,
          variable: h.variable,
          groupBy: h.groupBy,
          ...
          hiddenFieldsCount:
            h.variables?.filter((v) => v.isHidden)?.length || 0,
        }))
      );
  ...
  useEffect(() => {
    if (dqEmail?.mapRules && !burstField && !emailField) {
      try {
        ...
        if (burstElement) {
          console.log(
            "Loading existing burst field:",
            burstElement.textContent
          );
          setBurstField(burstElement.textContent);
        }
        if (emailElement) {
          console.log(
            "Loading existing email field:",
  ...
  useEffect(() => {
    if (
      ...
      const { mappings, updatedHierarchy } = parseExistingMappings(
        dqEmail.mapRules,
        parsedHierarchy,
        mapViewColumns,
        htmlTemplate
      ...
      if (Object.keys(mappings).length > 0) {
        ...
        console.log(
          "About to call setParsedHierarchy with:",
          updatedHierarchy.map((h) => ({
            collection: h.collection,
            variable: h.variable,
            groupBy: h.groupBy,
        ...
        console.log(
          "No mappings found in existing XML, marking as loaded anyway"
        );
        setExistingMappingsLoaded(true);
      }
    }
  ...
  useEffect(() => {
    console.log(
      "parsedHierarchy state changed:",
      parsedHierarchy.map((h) => ({
        collection: h.collection,
        variable: h.variable,
        groupBy: h.groupBy,
      }))
    ...
    while (i < original.length || j < updated.length) {
      if (i >= original.length) {
        ...
        if (
          originalTrimmed &&
          updatedTrimmed &&
          (originalTrimmed.includes(updatedTrimmed) ||
            updatedTrimmed.includes(originalTrimmed) ||
  ...
  const XMLDiffViewer = ({ originalXML, newXML, title }) => {
    ...
    const highlightXMLSyntax = (line, type) => {
      ...
      while ((tagMatch = tagRegex.exec(contentLine)) !== null) {
        ...
        if (tagNameMatch) {
          const tagNameEnd =
            tagContent.indexOf(tagNameMatch[1]) + tagNameMatch[1].length;
          tagParts.push(
        ...
        while ((attrMatch = attrRegex.exec(tagContent)) !== null) {
          ...
          tagParts.push(
            <span
              key={`attr-value-${attrMatch.index}`}
        ...
        parts.push(<span key={`tag-${tagMatch.index}`}>{tagParts}</span>);
        ...
        <div className="bg-gray-50 border rounded-lg overflow-hidden">
          <div className="max-h-96 overflow-auto font-mono text-sm">
            {diff.map((item, index) => (
              ...
              >
                <div
                  className={`w-12 px-2 py-1 text-right border-r text-gray-400 text-xs ${
                    item.type === "added"
                      ? "bg-green-100"
                      : item.type === "removed"
                      ? "bg-red-100"
  ...
  const parseTemplateHierarchy = (template) => {
    ...
    const hierarchy = matches.map((m, index) => {
      ...
      const instanceVariables = Array.from(templateVars)
        .filter((v) => v.startsWith(m.variable + "."))
        .map((v) => ({
          templateVar: v,
          fieldName: v.split(".")[1],
    ...
    hierarchy.forEach((level, index) => {
      ...
      if (!groupByValue) {
        ...
        if (firstVariable && mappings[firstVariable.templateVar]) {
          ...
          groupByValue = "ID";
          console.warn(
            `No groupBy value found for level ${index + 1} (${
              level.collection
            }), defaulting to 'ID'`
          );
        }
      ...
      level.variables.forEach((variable) => {
        const mappedColumn =
          mappings[variable.templateVar] || variable.fieldName;
  ...
  useEffect(() => {
    console.log(
      "Template parsing useEffect triggered, existingMappingsLoaded:",
      existingMappingsLoaded
    );
    ...
    if (!existingMappingsLoaded || !dqEmail?.mapRules) {
      ...
      console.log(
        "Skipping hierarchy update - existing mappings already loaded"
      );
    }
    ...
    if (
      ...
      (!dqEmail?.mapRules || !existingMappingsLoaded)
    ) {
      ...
      hierarchy.forEach((level) => {
        level.variables.forEach((variable) => {
          ...
          const similarColumn = mapViewColumns?.find((col) =>
            col.columnName
              .toLowerCase()
              .includes(variable.fieldName?.toLowerCase() || "")
          );
          initialMappings[variable.templateVar] =
            similarColumn?.columnName || "";
        });
      });
    ...
    if (mapViewColumns && (!burstField || !emailField)) {
      if (!burstField) {
        ...
        const burstColumn = mapViewColumns.find((col) =>
          ["burst", "burstid", "groupid", "recipient", "recipientid"].includes(
      ...
      if (!emailField) {
        ...
        const emailColumn = mapViewColumns.find((col) =>
          ["email", "emailaddress", "mail", "mailadress"].includes(
            col.columnName.toLowerCase()
          )
        );
    ...
    dqEmail?.mapRules,
    existingMappingsLoaded,
  ]); // Added existingMappingsLoaded dependency
  ...
  useEffect(() => {
    ...
    if (
      ...
      const xml = generateXMLFromHierarchy(
        parsedHierarchy,
        columnMappings,
        burstField,
        emailField
      ...
      if (parentLevelData) {
        parentLevelData.variables.forEach((variable) => {
          ...
          if (mappedColumn) {
            ...
            const columnInfo = mapViewColumns.find(
              (col) => col.columnName === mappedColumn
            );
            if (columnInfo) {
              // Check if not already added
              if (
                !availableColumns.find(
                  (existing) => existing.columnName === columnInfo.columnName
                )
              ) {
  ...
  const reparseTemplateAndUpdateVisibility = useCallback((templateText) => {
    ...
    setParsedHierarchy((currentHierarchy) => {
      ...
      console.log(
        "Re-evaluating field visibility and detecting new template variables"
      );
      ...
      const updatedHierarchy = currentHierarchy.map((level) => {
        ...
        const updatedVariables = level.variables
          .filter((variable) => {
            const isInTemplate = currentTemplateVars.has(variable.templateVar);
            const isMapped =
              columnMappings[variable.templateVar] &&
              columnMappings[variable.templateVar] !== "";

            console.log(
              `Checking field ${variable.templateVar}: inTemplate=${isInTemplate}, isMapped=${isMapped}, isHidden=${variable.isHidden}`
            );

            // Remove if: hidden + not in template + not mapped
            if (variable.isHidden && !isInTemplate && !isMapped) {
              console.log(
                `Removing orphaned hidden field: ${variable.templateVar} (not in template and not mapped)`
              );
              return false;
            }
          ...
          .map((variable) => {
            ...
            const isNowHidden = isFieldHidden(
            ...
            if (wasHidden !== isNowHidden) {
              console.log(
                `Field visibility changed: ${variable.templateVar} was ${
                  wasHidden ? "hidden" : "visible"
                }, now ${isNowHidden ? "hidden" : "visible"}`
              );
            }
        ...
        const existingVars = new Set(
          updatedVariables.map((v) => v.templateVar)
        );
        const newVariables = [];
        ...
        [...currentTemplateVars].forEach((templateVar) => {
          ...
          if (
            templateVar.startsWith(level.variable + ".") &&
            !existingVars.has(templateVar)
          ) {
            const fieldName = templateVar.split(".")[1];
            console.log(
              `Found new template variable: ${templateVar} (field: ${fieldName}) for level ${level.collection}`
            );
        ...
        if (newVariables.length > 0) {
          console.log(
            `Adding ${newVariables.length} new template variables to level ${level.collection}`
          );
        }
  ...
  const handleSave = () => {
    if (onSave) {
      ...
      const hierarchyString = parsedHierarchy
        .map((level) => level.collection)
        .join(">");
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                The field that determines how recipients are grouped for email
                bursts
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow border p-6">
          ...
          <div className="space-y-6">
            {parsedHierarchy.map((level, levelIndex) => {
              ...
              return (
                <div
                  key={`${levelIndex}-${level.collection}-${
                    level.groupBy || "no-groupby"
                  }`}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <select
                          key={`groupby-${levelIndex}-${
                            level.groupBy || "empty"
                          }`}
                          value={level.groupBy || ""}
                          onChange={(e) =>
                            handleGroupByChange(levelIndex, e.target.value)
                          }
                        ...
                        {levelIndex > 0 && (
                          <div className="text-xs mt-1">
                            {getAvailableGroupByColumns(levelIndex).length ===
                            0 ? (
                              <span className="text-red-600 bg-red-50 px-2 py-1 rounded">
                                ⚠️ No fields available from parent levels
                              </span>
                            ) : level.groupBy &&
                              !getAvailableGroupByColumns(levelIndex).find(
                                (col) => col.columnName === level.groupBy
                              ) ? (
                              <span className="text-orange-600 bg-orange-50 px-2 py-1 rounded">
                                ⚠️ Selected field not available from parent
                                levels
                              </span>
                            ) : level.groupBy ? (
                              ...
                              <span className="text-gray-500">
                                {getAvailableGroupByColumns(levelIndex).length}{" "}
                                fields available from parent levels
                              </span>
                            )}
                          </div>
                        )}
                  ...
                  <div className="grid gap-3">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {variable.isHidden && (
                              <button
                                onClick={() =>
                                  removeHiddenField(levelIndex, varIndex)
                                }
                    ...
                    <div className="bg-gray-50 rounded border border-dashed border-gray-300 p-3">
                      ...
                      <div className="text-xs text-gray-500 mt-2 ml-7">
                        Add a field that exists in the view but is not displayed
                        in the email template. These fields can be used for
                        grouping in child levels.
                        <span className="block mt-1 font-medium">
                          {getUnmappedColumns(levelIndex).length} unmapped
                          columns available
                        </span>
                      </div>
                    </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">
                No ^for directives found
              </h3>
              <p className="text-sm text-yellow-700 mt-1">
                Add ^for=&quot;variable in collection&quot; directives to your
                HTML template to enable automatic XML generation.
              </p>
            </div>
          </div>
        </div>
        ...
        {showDiffView ? (
          ...
          dqEmail.mapRules && generatedXML ? (
            ...
            <div className="text-center py-8 text-gray-500">
              {!dqEmail.mapRules
                ? "No existing MapRules to compare"
                : "Generate XML to see diff"}
            </div>
          )
        ) : (
          // Side-by-Side View
                  ...
                  <div className="w-8 bg-gray-100 border-r text-xs text-gray-400 font-mono">
                    {dqEmail.mapRules ? (
                      formatXml(dqEmail.mapRules)
                        .split("\n")
                        .map((_, i) => (
                          <div key={i} className="px-1 text-right">
                            {i + 1}
                          </div>
                        ))
                    ) : (
              ...
              <div className="bg-blue-50 border border-blue-200 rounded p-4 max-h-96 overflow-y-auto">
                <div className="flex">
                  ...
                  <pre className="text-xs font-mono text-blue-800 whitespace-pre-wrap px-3 flex-1">
                    {generatedXML
                      ? formatXml(generatedXML)
                      : "Template parsing will generate XML here..."}
                  </pre>
                </div>
              </div>
            </div>
